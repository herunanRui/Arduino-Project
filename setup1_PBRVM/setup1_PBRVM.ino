#include <SoftwareSerial.h> //library that can be used in thermal printer
...
SoftwareSerial mySerial(10, 11); // RX, TX of thermal printer
...
void setup() {
  ...
  pinMode(butClaim, INPUT);

  pinMode(butPrint, INPUT);
...
void loop() {
  ...
  int butPrintVal = digitalRead(butPrint);
  ...
  if (irSensorValue == LOW) {
        ...
        delay(300); // Sound the buzzer for 300 m.second
        ...
        digitalWrite(buzzerPin, HIGH);
        delay(700); // Sound the buzzer for 300 m.second
        digitalWrite(buzzerPin, LOW); // Turn off the buzzer
        digitalWrite(buzzerPin, HIGH);
        delay(700); // Sound the buzzer for 300 m.second
        digitalWrite(buzzerPin, LOW); // Turn off the buzzer
        ...
        if ( butClaimVal == HIGH) {
              ...
              delay(4000);
                ...
                // if the user click yes or danote the inserted plastic bottle
                if (butClaimVal == HIGH) { 
                lcd.clear();
                lcd.setCursor(0, 0);
                lcd.print("Thank You");
                ...
                // if the user click no or does not want to donate the inserted plastic bottle
                else if (butPrintVal == HIGH) {
                lcd.clear();
                lcd.setCursor(0, 0);
                lcd.print("Inserted: ");
                lcd.setCursor(12, 0);
        ...
        //if the user want to print out his/her reward
        else if (butPrintVal == HIGH){
                delay(500);
                lcd.clear();
                lcd.setCursor(0, 0);
                lcd.print("Are you sure you");
                lcd.setCursor(0, 1);
                lcd.print("want to print?");
                delay(4000);

                // if the user click yes or wants to print his/her points
                if (butClaimVal == HIGH) { 
                mySerial.println();
                mySerial.println("\x1B\x14");
                mySerial.println("  PBRVM");
                mySerial.println("__________");
                mySerial.println();
                mySerial.print("Inserted Bottle: ");
                mySerial.println(detectionCount);
                mySerial.println("Total point: ");
                mySerial.print(plasticValue);
                mySerial.println();
                mySerial.println();
                mySerial.println("Claim your reward in canteen");
                mySerial.println("Select a reward base on your total points");
                mySerial.println();
